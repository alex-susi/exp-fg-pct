...
get_game_with_lineups <- function(game_id) {
  ...
  home_starters <- as.character(
    rot$HomeTeam %>% filter(IN_TIME_REAL == "0") %>% pull(PERSON_ID))
  away_starters <- as.character(
    rot$AwayTeam %>% filter(IN_TIME_REAL == "0") %>% pull(PERSON_ID))
...
engineer_features <- function(pbp) {
  pbp %>%
    ...
    mutate(
      ...
      #time_since_last_event = max(total_elapsed - lag(total_elapsed, n = 1), 0),
      time_since_last_event = pmax(coalesce(total_elapsed - lag(total_elapsed), 0), 0),
      ...
      possession_end = as.integer(
        fgm == 1 |
          ...
          (action_type == "jumpball" & 
             (# jumpball immediately after period start => possession ends
               (lag(action_type) == "period" & lag(sub_type) == "start") |
                 # otherwise: ends iff offense_team_id != team_id (or either is NA)
    ...
    mutate(
      # IMPORTANT: vectorized delta (NOT max())
      time_since_last_event = pmax(coalesce(total_elapsed - lag(total_elapsed), 0), 0),
...
## ═════════════════════════════════════════════════════════════════════════════
## STAGE 4 — SECONDS SINCE PLAY START
## ═════════════════════════════════════════════════════════════════════════════

compute_sec_since_play_start <- function(pbp) {
  pbp %>%
    ...
    mutate(
      ...
      #   out <- numeric(n())
      #   for (i in seq_len(n())) {
      #     if (!is.na(pe[i]) && pe[i] == 1) last_end <- t[i]
      #     out[i] <- last_end
      #   }
      #   out
      # },
      # sec_since_play_start = pmax(total_elapsed - last_poss_end_time, 0,
      #                             na.rm = TRUE),
      # sec_since_play_start = if_else(is.na(sec_since_play_start), 0,
      #                                sec_since_play_start),
      
      sec_since_play_start = cumsum(time_since_last_event)) %>%
    ungroup()
}
...
link_shooting_fouls <- function(pbp) {
  ...
  foul_trips <- lapply(foul_rows, function(fi) {
    ...
    tibble(foul_row_id = fi, foul_game_id = gid,
           foul_drawn_by = drawn_by,
           fouling_player_id = fouling_player,
           fouling_team_id = as.character(foul_evt$team_id),
           n_ft_awarded = as.integer(n_ft), n_ft_made = as.integer(ft_made),
           is_and1 = is_and1, preceding_fga_idx = preceding_fga_idx)
  }) %>% bind_rows()
...
## ═════════════════════════════════════════════════════════════════════════════
## 06 — MATCHUP DATA + DEFENDER IMPUTATION   ===================================
## ═════════════════════════════════════════════════════════════════════════════
...
pull_game_matchups <- function(game_id) {
  ...
  home_df <- tryCatch(
    as.data.frame(res$home_team_player_matchups),
    error = function(e) NULL
  )
  
  away_df <- tryCatch(
    as.data.frame(res$away_team_player_matchups),
    error = function(e) NULL
  )
...
run_sanity_check <- function(pbp, matchups, fga_idx, verbose = TRUE) {
  ...
  comparison <- matchup_shares %>%
    ...
    mutate(
      matchup_fga   = coalesce(matchup_fga, 0L),
...
## ════════════════════════════════════════════════════════════════════════════
## STAGE 8 — TRACKING AGGREGATES (revised)   ==================================
##
...
## to rim shots vs threes by the same player — with ZERO additional API calls.
##
...
get_tracking_aggregates <- function(season_str) {
  ...
  cs_profiles <- cs_raw %>%
    transmute(
      PLAYER_ID = as.character(PLAYER_ID),
      type = case_when(
        general_range == "Catch and Shoot" ~ "cs",
...
impute_shot_context <- function(pbp, tracking, seed = 42, verbose = TRUE) {
  ...
  jump_shots <- pbp[fga_idx, ] %>%
    ...
    mutate(
      pid        = as.character(player1_id),
      is_3_flag  = as.integer(is_three == 1),
      definite_non_cs = grepl(non_cs_pattern, descriptor, ignore.case = TRUE) &
        !is.na(descriptor)
    )
  ...
  for (i in fga_idx) {
    ...
    if (!is.null(dl)) {
      ...
      # No tracking data for this player → league average
    ...
    # Fastbreaks involve transition defense → more open shots
    if (is_fb) {
      # Reduce weight on tight bins, boost open bins
      weights[1] <- weights[1] * 0.3   # 0-2ft: rare in fastbreak
      weights[2] <- weights[2] * 0.5   # 2-4ft: reduced
      weights[3] <- weights[3] * 1.2   # 4-6ft: slightly boosted
      weights[4] <- weights[4] * 2.0   # 6+ft: doubled
      weights <- weights / sum(weights)
      n_fb_boost <- n_fb_boost + 1L
    }
    
    # Sample bin from distribution
    u <- runif(1)
    cum <- cumsum(weights)
    bin_idx <- which(u <= cum)[1]
    if (is.na(bin_idx)) bin_idx <- 4L
    
    # Sample continuously within the bin (avoids discrete clumping)
  ...
  if (verbose) {
    ...
    message("      Def dist — type-specific=", n_def_type,
    ...
    for (fam in c("rim", "j2", "j3")) {
      ...
      if (nrow(sub) > 0) {
        message(
          "      ",
          sprintf("%-3s: n=%-4d  mean_def_dist=%4.2f  mean_p_cs=%4.3f  cs_rate=%4.3f",
                  fam, nrow(sub),
...
assemble_and_scale <- function(pbp, schedule, bio, tracking, seed = 42) {
  ...
  pbp <- pbp %>%
    mutate(player_idx  = as.integer(factor(player1_id)),
           defender_idx = as.integer(factor(imputed_def_id)),
           defteam_idx = as.integer(factor(def_team_abbr)))

  # Scale continuous features within shot family
  # (for test subsets with 1 game, sd can be 0 or NA — guard against that)
  ...
  attr(pbp, "player_map")     <- fga_rows %>%
    distinct(player_idx, player1_id, player_name) %>% arrange(player_idx)
...
build_season_pbp <- function(season_year, game_ids = NULL, cache_dir = "cache") {
  ...
  ## ── 4. Seconds since play start ─────────────────────────────────────────
  message("─── Step 4: Seconds since play start ───")
  pbp <- compute_sec_since_play_start(pbp)
  med_t <- round(median(pbp$sec_since_play_start[pbp$fga == 1], na.rm = TRUE), 1)
  message("  Median (FGA): ", med_t, "s")
