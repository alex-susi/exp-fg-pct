...
standardize_live_pbp <- function(data) {
  ...
  # ----- coordinate aliases -----
  # Your pipeline uses x_legacy/y_legacy. hoopR's rename snippet references xLegacy/yLegacy.
  if ("x_legacy" %in% names(data) && !"xLegacy" %in% names(data)) data$xLegacy <- data$x_legacy
  if ("y_legacy" %in% names(data) && !"yLegacy" %in% names(data)) data$yLegacy <- data$y_legacy
  if ("xLegacy" %in% names(data) && !"x_legacy" %in% names(data)) data$x_legacy <- data$xLegacy
  if ("yLegacy" %in% names(data) && !"y_legacy" %in% names(data)) data$y_legacy <- data$yLegacy
  ...
  # Keep legacy coords available for your engineer_features() even if locX/locY got created
  if (!"x_legacy" %in% names(data) && "locX" %in% names(data)) data$x_legacy <- data$locX
  if (!"y_legacy" %in% names(data) && "locY" %in% names(data)) data$y_legacy <- data$locY
  
  # Optional: wrap in hoopR's classing if available (won't break your pipeline if missing)
  data <- tryCatch(
    hoopR:::make_hoopR_data("NBA Game Play-by-Play Information from NBA.com", Sys.time())(data),
...
.PTSHOT_HEADERS <- add_headers(`User-Agent` = paste0("Mozilla/5.0 (Windows NT 10.0; Win64; x64) ",
...
.nba_stats_ptshot_direct <- function(
    date_from            = "",
    date_to              = "",
    season               = "",
...
.fetch_combo_batch <- function(combos_df, date_mdy, season_str) {
  ...
  for (i in seq_len(nrow(combos_df))) {
    ...
    q <- list(
      DateFrom = date_mdy, DateTo = date_mdy,
...
engineer_features <- function(pbp) {
  pbp %>%
    ...
    { 
      col_order <- c(
        "game_id","event_num","clock","period","period_type","action_type","sub_type","qualifiers",
        "player1_id","x","y","offense_team_id","home_score","away_score","order","x_legacy","y_legacy",
        "is_field_goal","side","description","person_ids_filter","team_id","team_tricode","descriptor",
        "jump_ball_recovered_name","jump_ball_recoverd_person_id","player_name","player_name_i",
        "jump_ball_won_player_name","jump_ball_won_person_id","jump_ball_lost_player_name",
        "jump_ball_lost_person_id","area","area_detail","shot_distance","shot_result","shot_action_number",
        "rebound_total","rebound_defensive_total","rebound_offensive_total","points_total",
        "assist_player_name_initial","assist_person_id","assist_total","turnover_total","steal_player_name",
        "steal_person_id","official_id","foul_personal_total","foul_technical_total","foul_drawn_player_name",
        "foul_drawn_person_id","block_player_name","block_person_id","player2_id","player3_id",
        "home_p1","home_p2","home_p3","home_p4","home_p5","away_p1","away_p2","away_p3",
        #"locX","locY","home_p1","home_p2","home_p3","home_p4","home_p5","away_p1","away_p2","away_p3",
        "away_p4","away_p5","fga","fgm","fta","ftm","shot_angle","is_three","is_dunk","is_rim","is_jump",
        "shot_family","shot_type","elapsed_seconds","game_clock_sec","period_start","total_elapsed",
        "is_turnover","is_defrebound","is_shootingfoul","is_2ndchance","is_fastbreak","is_fromturnover",
        "is_last_freethrow","ft_of_n","period_end","possession_end","possession_id","time_since_last_event",
...
build_season_pbp_inChunks <- function(season_year, chunk_size = 5,
  ...
  season_str <- year_to_season(season_year)
  message("\n══════════════════════════════════════════════════════")
  message("  CHUNKED BUILD: ", season_str, "  |  ", chunk_size, " game-date(s) per chunk")
  message("══════════════════════════════════════════════════════\n")
  ...
  for (ch in seq_len(n_chunks)) {
    ...
    csv_path <- paste0("pbp_", season_year, "_", season_year+1, "/pbp_", date_tag, ".csv")
  ...
  write_csv(pbp_all %>%
              select(-any_of(c("on_court_defenders", "x", "y"))) %>%
...
pbp_all <- read.csv("pbp_2024_2025/pbp_2024_2025_FULL.csv") %>% 
  rename(shot_angle = angle) %>%
...
problems(pbp_test) %>% as.data.frame()  # shows exact file, row, column, expected vs actual type
