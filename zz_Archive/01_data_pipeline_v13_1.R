...
get_game_level_tracking <- function(schedule, season_str, cache_dir = "cache") {
  ...
  for (g_idx in seq_along(date_groups)) {
    ...
    if (use_parallel) {
      ...
      grp_df <- bind_rows(chunk_results)
    ...
    if (!is.null(grp_df) && nrow(grp_df) > 0) {
      ...
      # Safety: warn if any rows couldn't be mapped
...
if (TRUE) {
  ...
  chunk_ids <- sort(rep(seq_len(N_WORKERS), length.out = nrow(test_combos)))
  ...
  grp_raw <- dplyr::bind_rows(chunk_results)
...
# period only reconciles w/ brunson
# period/shot clock/dribble range/defender distance reconciles w/ brunson

lt10_combo <- expand.grid(period               = seq_len(4),
                          shot_clock_range     = SHOT_CLOCK_RANGES,
                          dribble_range        = DRIBBLE_RANGES,
                          close_def_dist_range = DEF_DIST_RANGES,
                          general_range        = GENERAL_RANGES,
...
#tracking_20241022_testLT10 <- .fetch_combo_batch(lt10_combo, 
tracking_20241022_testLT10_allranges <- get_game_level_tracking(
...
tracking_20241022_testLT10_allranges %>% filter(PLAYER_ID == "1628973") %>%
#tracking_20241022_testLT10 %>% filter(PLAYER_NAME == "Jalen Brunson") %>%
  arrange(period, shot_clock_range) %>% 
  group_by(PLAYER_ID, period) %>% 
