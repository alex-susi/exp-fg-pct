...
.nba_stats_ptshot_direct <- function(date_from            = "",
  ...
  q <- list(DateFrom          = date_from,
            ...
            GeneralRange = "", Height = "", LastNGames = 0,
...
##   SC_BLEND_EPS of any boundary we SUM the two adjacent buckets'
...
impute_shot_context <- function(pbp, game_tracking, seed = 42, verbose = TRUE) {
  ...
  #   - "ALL" (NA shot_clock)
  #
  # v9: eps parameter allows wider blending for live-ball possessions.
  ...
  for (gk in unique_groups) {
    ...
    if (is.null(wl_cache[[gk]])) {
      ...
      wl  <- resolve_wl(pgd_per_base, g_sc, pgd_lookup)
      src <- "pgd-period"
      
      if (is.null(wl)) { 
        wl <- resolve_wl(pgd_0_base, g_sc, pgd_lookup)
        src <- "pgd"
        }
      if (is.null(wl)) { 
        wl <- resolve_wl(ps_per_base, g_sc, ps_lookup)
        src <- "season-period" 
        }
      if (is.null(wl)) { 
        wl <- resolve_wl(ps_0_base, g_sc, ps_lookup)
        src <- "season"        
        }
      if (is.null(wl)) { 
        wl <- resolve_wl(league_per_base, g_sc, league_lookup)
        src <- "league-period" 
        }
      if (is.null(wl)) { 
        wl <- resolve_wl("0|", g_sc, league_lookup)
        src <- "league"        
        }
      if (is.null(wl)) { 
        wl <- league_lookup[["0|ALL"]]
        src <- "league-all"    
        }
  ...
  for (j in seq_len(n_fga)) {
    ...
    # ── Override C (v12): force 0 Dribbles for tip / alley-oop / putback ──
    ...
    if (fga_meta$definite_non_cs[j] && 
      ...
      wl_j <- if (!is.null(wl_cache[[gk_j]])) {
        ...
        if (is.null(wl_jj)) {
          wl_jj <- resolve_wl(pgd_0_j, g_sc_j, pgd_lookup)
          }
        if (is.null(wl_jj)) {
          wl_jj <- resolve_wl(ps_per_j, g_sc_j, ps_lookup)
          }
        if (is.null(wl_jj)) {
          wl_jj <- resolve_wl(ps_0_j, g_sc_j, ps_lookup)
          }
        if (is.null(wl_jj)) {
          wl_jj <- resolve_wl(paste0(g_per_j, "|"), g_sc_j, league_lookup)
          }
        if (is.null(wl_jj)) {
          wl_jj <- resolve_wl("0|", g_sc_j, league_lookup)
          }
        if (is.null(wl_jj)) {
          wl_jj <- league_lookup[["0|ALL"]]
          }
        wl_jj
      }
    ...
    if (fga_meta$is_contact[j] == 1L) {
      if (!is.na(res_dd[j]) && res_dd[j] != "0-2 Feet - Very Tight") {
  ...
  # ═══════════════════════════════════════════════════════════════════════════
  # STAGE 2b: GROUP-LEVEL DEF_DIST RE-BALANCE AFTER CONTACT OVERRIDES
  # ═══════════════════════════════════════════════════════════════════════════
  #
  # After forcing contact shots to "0-2 Feet", the def_dist distribution
  # within each group may be skewed — we've added extra shots to the 0-2
  # bucket.  Re-sample non-contact shots from the adjusted marginal
  # (original minus contact-forced slots) to preserve distributional fidelity.
  # This mirrors Override A's dribble re-sampling logic.

  n_defdist_resample <- 0L
  ...
  for (gk in unique_groups) {
    ...
    # Identify contact and non-contact shots in this group
    contact_mask     <- grp_mask[fga_meta$is_contact[grp_mask] == 1L]
    non_contact_mask <- grp_mask[fga_meta$is_contact[grp_mask] != 1L]
    n_contact_grp    <- length(contact_mask)
    n_noncontact_grp <- length(non_contact_mask)

    if (n_contact_grp == 0L || n_noncontact_grp == 0L) next

    # Get the attempt weight matrix for this group
    wl  <- wl_cache[[gk]]$wl
    if (is.null(wl)) next
    ...
    w_att <- if (g_is3 == 1L) wl$w3 else wl$w2
    if (sum(w_att) == 0L) w_att <- wl$w2 + wl$w3
    if (sum(w_att) == 0L) next

    # Original def_dist marginal (column sums)
    dd_marginal <- colSums(w_att)

    # Subtract contact shots forced into "0-2 Feet"
    dd_marginal["0-2 Feet - Very Tight"] <-
      max(dd_marginal["0-2 Feet - Very Tight"] - n_contact_grp, 0L)

    if (sum(dd_marginal) > 0L) {
      # Re-sample non-contact shots' def_dist from adjusted marginal
      new_dd <- sample(DEF_DIST_RANGES, n_noncontact_grp, replace = TRUE,
                       prob = dd_marginal / sum(dd_marginal))
      res_dd[non_contact_mask] <- new_dd
      n_defdist_resample <- n_defdist_resample + n_noncontact_grp
    }
  }
  ...
  if (verbose) {
    ...
    message("    Post-hoc overrides:  descriptor-conflict=", n_desc_conflict,
            ...
            "  defdist-resample=", n_defdist_resample,
...
## ═════════════════════════════════════════════════════════════════════════════
## cleanup_lineups.R
## v12 hotfix — Reprocess cached pbp_raw RDS files to fix the substitution
## ordering bug (sub_type "in" arriving before "out" at the same game clock).
##
## Usage:
##   source("cleanup_lineups.R")
##   # Processes all RDS files in the target folder and re-saves in place.
## ═════════════════════════════════════════════════════════════════════════════
## ─────────────────────────────────────────────────────────────────────────────
## cleanup_lineups()
##
## Takes a single pbp_raw RDS path, recomputes home_p1–away_p5 using the
## v12 out-before-in reordering fix, and overwrites the file.
##
## Key design decisions:
##   - Starters are recovered from the FIRST row of each game (set from
##     rotation API data in get_game_with_lineups; always correct since the
##     bug only affects mid-game sub processing).
##   - home_team_id is derived (not stored in pbp_raw) by matching players
##     in substitution "out" events against the starting home lineup.
##   - Each game within the chunk is processed independently.
## ─────────────────────────────────────────────────────────────────────────────

cleanup_lineups <- function(rds_path) {
  
  message("Processing: ", basename(rds_path))
  pbp_chunk <- readRDS(rds_path)
  game_ids  <- unique(pbp_chunk$game_id)
  message("  Games found: ", length(game_ids))
  
  pad5 <- function(v) c(v, rep(NA_character_, max(0, 5 - length(v))))[1:5]
  
  fixed_games <- lapply(game_ids, function(gid) {
    
    game_pbp <- pbp_chunk %>% filter(game_id == gid)
    
    # ── Recover starters from first row (always correct) ───────────────
    first_row  <- game_pbp[1, ]
    home_start <- c(first_row$home_p1, first_row$home_p2, first_row$home_p3,
                    first_row$home_p4, first_row$home_p5)
    away_start <- c(first_row$away_p1, first_row$away_p2, first_row$away_p3,
                    first_row$away_p4, first_row$away_p5)
    
    cur_home <- pad5(home_start)
    cur_away <- pad5(away_start)
    
    # ── Derive home_team_id from sub events ────────────────────────────
    subs <- game_pbp %>%
      filter(!is.na(action_type),
             tolower(action_type) == "substitution") %>%
      mutate(player1_id = as.character(player1_id),
             team_id    = as.character(team_id))
    
    home_ids <- cur_home[!is.na(cur_home)]
    away_ids <- cur_away[!is.na(cur_away)]
    
    home_team_id <- subs %>%
      filter(player1_id %in% home_ids) %>%
      pull(team_id) %>%
      unique()
    home_team_id <- home_team_id[1]
    
    if (is.na(home_team_id)) {
      away_team_id <- subs %>%
        filter(player1_id %in% away_ids) %>%
        pull(team_id) %>%
        unique()
      away_team_id <- away_team_id[1]
      all_teams    <- unique(subs$team_id)
      home_team_id <- setdiff(all_teams, away_team_id)[1]
    }
    
    if (is.na(home_team_id)) {
      message("    ", gid, ": no subs found, skipping")
      return(game_pbp)
    }
    
    # ── Pre-index all sub rows by (period, clock) ──────────────────────
    n      <- nrow(game_pbp)
    is_sub <- !is.na(game_pbp$action_type) &
      tolower(game_pbp$action_type) == "substitution"
    
    sub_batch_key <- ifelse(is_sub,
                            paste0(game_pbp$period, "|", game_pbp$clock),
                            NA_character_)
    
    # ── Replay lineup tracking loop (batch-process subs) ───────────────
    home_lineups     <- vector("list", n)
    away_lineups     <- vector("list", n)
    processed_clocks <- list()
    ...
    for (i in seq_len(n)) {
      ...
      if (is_sub[i]) {
        ...
        if (is.null(processed_clocks[[batch_key]])) {
          # First sub at this (period, clock) — batch-process ALL subs here.
          batch_idx <- which(sub_batch_key == batch_key)
          
          # Two passes: all "out" first, then all "in"
          for (pass in c("out", "in")) {
            for (bi in batch_idx) {
              sub_dir <- tolower(as.character(game_pbp$sub_type[bi]))
              if (!grepl(pass, sub_dir)) next
              
              pid    <- as.character(game_pbp$player1_id[bi])
              tid    <- as.character(game_pbp$team_id[bi])
              lineup <- if (tid == home_team_id) cur_home else cur_away
        ...
        # Sub rows (first or subsequent in batch) get the post-batch
        # lineup, stamped below.
      }
    ...
    # ── Overwrite lineup columns ───────────────────────────────────────
    game_pbp$home_p1 <- map_chr(home_lineups, ~ .x[1])
    game_pbp$home_p2 <- map_chr(home_lineups, ~ .x[2])
    game_pbp$home_p3 <- map_chr(home_lineups, ~ .x[3])
    game_pbp$home_p4 <- map_chr(home_lineups, ~ .x[4])
    game_pbp$home_p5 <- map_chr(home_lineups, ~ .x[5])
    game_pbp$away_p1 <- map_chr(away_lineups, ~ .x[1])
    game_pbp$away_p2 <- map_chr(away_lineups, ~ .x[2])
    game_pbp$away_p3 <- map_chr(away_lineups, ~ .x[3])
    game_pbp$away_p4 <- map_chr(away_lineups, ~ .x[4])
    game_pbp$away_p5 <- map_chr(away_lineups, ~ .x[5])
    
    game_pbp
  })
  
  pbp_fixed <- bind_rows(fixed_games)
  
  # ── Diagnostics ──────────────────────────────────────────────────────
  lineup_cols <- c("home_p1","home_p2","home_p3","home_p4","home_p5",
                   "away_p1","away_p2","away_p3","away_p4","away_p5")
  na_counts <- colSums(is.na(pbp_fixed[, lineup_cols]))
  total_nas <- sum(na_counts)
  if (total_nas > 0) {
    message("  ⚠ Residual NAs in lineup columns: ", total_nas)
    message("    ", paste(names(na_counts[na_counts > 0]),
                          na_counts[na_counts > 0],
                          sep = "=", collapse = "  "))
  } else {
    message("  ✓ No NAs in lineup columns")
  }
  
  saveRDS(pbp_fixed, rds_path)
  message("  ✓ Saved: ", basename(rds_path), "  (",
          nrow(pbp_fixed), " rows, ",
          length(game_ids), " games)\n")
...
## ═════════════════════════════════════════════════════════════════════════════
## Run over all RDS files in the cache folder
## ═════════════════════════════════════════════════════════════════════════════

cache_folder <- "cache/pbp_raw_2024_2025"
rds_files    <- sort(list.files(cache_folder, 
                                pattern = "\\.rds$", 
                                full.names = TRUE))

message("═══ Lineup cleanup: ", length(rds_files),
        " RDS file(s) in ", cache_folder, " ═══\n")
