...
# Count FGA per player per shot family (including zeros) 
player_vol <- expand_grid(player1_id = unique(shots$player1_id), 
  ...
  ungroup() %>% 
  # Standardize within each shot family
...
## ─── EPAA per-shot 90% credible intervals (players) ───
## Propagate a_player posterior uncertainty through inv_logit using
## each player's mean eta_context as a representative baseline.
...
epaa_per100_ci  <- bind_rows(lapply(1:3, function(t) {
  ...
  a_draws <- draws_mat[, cols, drop = FALSE]          # [n_draws × J]
  
  base_p    <- inv_logit(pb$mean_eta)                 # length-J vector
  skilled_p <- inv_logit(sweep(a_draws, 2, pb$mean_eta, "+"))  # [n_draws × J]
...
## ─── Points Saved Above Average per contest: 90% CI (defenders) ───
## Positive = good defense (opponent scored less than expected).
...
ggsave("graphs/twostage_calibration.png", 
...
caterpillar_plot <- function(df, family, name_col, n_col,
  ...
  # Use contests if available (defender effects), otherwise fga
  #n_col <- if ("contests" %in% names(effects_df)) "contests" else "fga"
  ...
  df_sub <- bind_rows(top, bot) %>%
    distinct(idx, .keep_all = TRUE) %>%          # dedupe if overlap
    arrange(desc(.data[[mean_col]])) %>%
...
ggsave("graphs/player_caterpillar.png", p_cat, width = 14, height = 8, dpi = 300)
ggsave("poster_CSAS/player_caterpillar.png", p_cat, width = 18, height = 5, dpi = 300)
ggsave("graphs/player_caterpillar.png", p_cat, width = 14, height = 3, dpi = 150)
...
ggsave("graphs/defender_caterpillar.png", def_cat, width = 14, height = 3, dpi = 150)
ggsave("poster_CSAS/defender_caterpillar.png", def_cat, width = 18, height = 5, dpi = 300)
...
ggsave("graphs/sigma_comparison.png", p_sigma, width = 12, height = 5, dpi = 150)
...
ggsave("graphs/xfg_recalibration.png", p_recal, width = 7, height = 6, dpi = 150)
...
ggsave("graphs/yoy.png", p_yoy, width = 7, height = 6, dpi = 150)
