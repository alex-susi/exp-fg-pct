...
.nba_stats_ptshot_direct <- function(
  ...
  q <- list(
    ...
    # Required defaults — API rejects calls without these
  ...
  for (attempt in seq_len(max_retries)) {
    ...
    if (sc == 200L) {
      ...
      # Empty result set — no shots matched this filter combination
...
##
## v3 optimised: pulls the JOINT distribution of period × shot_clock_range ×
## dribble_range × def_dist_range per player per game date.
##
## Speed improvements over the original 840-call/date loop:
##
##   1. ADAPTIVE PERIOD COUNT — uses game_status_text from the schedule to
##      determine the actual max period for each game date.  "Final" → 4
##      periods (480 calls), "Final/OT" → 5 (600), "Final/OT2" → 6 (720).
##      ~90% of dates have no OT → saves ~42% of calls season-wide.
##
##   2. DIRECT API CALLS — bypasses hoopR, hits stats.nba.com directly via
##      httr.  Eliminates hoopR's "df_list not found" crash on empty results
##      and avoids wasted retry overhead.
##
##   3. LOWER INTER-CALL DELAY — 0.4s instead of 0.6s.
##
##   4. EARLY PERIOD TERMINATION — if an OT period (5, 6, 7) returns 20
##      consecutive empty results, the rest of that period is skipped.
##
## Caching:
...
## Consecutive empty combos before skipping the rest of an OT period.
.EARLY_STOP_THRESHOLD <- 20L

get_game_level_tracking <- function(schedule, season_str, cache_dir = "cache") {

  season_tag     <- gsub("-", "", substr(season_str, 1, 7))
  ...
  combos_per_period <- n_sc * n_dr * n_dd    # 6 × 5 × 4 = 120
  ...
  message("  Total API calls: ", total_calls,
          "  (est. ", round(total_calls * 0.5 / 60, 1), " min @ 0.4s + overhead)")
  ...
  for (d_idx in seq_along(game_dates)) {
    ...
    message("  [", d_idx, "/", n_dates, "] ", date_ymd,
            " (", max_per, " per, ", date_n_combos, " calls) scraping…")
      ...
      for (sc in SHOT_CLOCK_RANGES) {
        for (dr in DRIBBLE_RANGES) {
          for (dd in DEF_DIST_RANGES) {
            ...
            # ── Early period termination (OT periods only) ──────────────
            if (period_skipped) {
              k <- k + 1L
              next
            }

            # ── Progress: remaining combos ──────────────────────────────
            remaining <- date_n_combos - k
            if (k %% 30 == 1 || k == date_n_combos) {
              elapsed <- as.numeric(difftime(Sys.time(), t_start, units = "secs"))
              rate    <- if (k > 1) elapsed / (k - 1) else 0.5
              eta_sec <- remaining * rate
              message("    combo ", k, "/", date_n_combos,
                      "  (", remaining, " left, ~",
                      round(eta_sec / 60, 1), " min remaining)",
                      "  P", per, " | ", sc, " | ", dr, " | ", dd)
            }

            # ── Direct API call (no hoopR, 0.4s delay) ──────────────────
            ...
            if (!is.null(df) && nrow(df) > 0) {
              ...
              consec_empty <- consec_empty + 1L
              if (per > 4L && consec_empty >= .EARLY_STOP_THRESHOLD) {
                message("    ⏭ Period ", per, ": ", consec_empty,
                        " consecutive empty combos — skipping remainder")
                period_skipped <- TRUE
              }
            }
...
test3 <- build_season_pbp(2024, game_ids = schedule$game_id[420])
