...
shared_features <- c("shot_distance",     # feet from basket
                     ...
                     "wingspan_diff",     # shooter_ws - defender_ws (inches)
...
shots <- shots %>%
  mutate(height_diff      = coalesce(height_diff, 0),
         wingspan_diff    = coalesce(wingspan_diff, 0),
         player_min_game  = coalesce(player_min_game, 0),
...
ggsave("xgb_logloss_baseline_vs_tuned.png", p_tuning_comparison,
...
ggsave("xgb_logloss_full_vs_reduced.png", p_feature_comparison,
       width = 10, height = 14, dpi = 150)
...
# data checking (4 3-pointers were counted as rim bc of area)
# some dunks are being counted as mid ranges bc distance appears incorrect
# check other shots w similar x,y coords

shots %>% 
  filter(shot_family == "j2", is_dunk == 1) %>% 
...
tracking_20241112 %>% filter(PLAYER_ID == 1628374) %>% 
  arrange(period, shot_clock_range)

matchups_20241112_20241116 %>% 
  filter(game_id == "0022400006",
         person_id == 1628374)

matchups_20241112_20241116 %>% 
  filter(game_id == "0022400006",
         person_id == 1628374) %>% 
  group_by(matchups_player_slug) %>% 
  summarise(n_fga = sum(matchup_field_goals_attempted),
            n_3fga = sum(matchup_three_pointers_attempted),
            n_2fga = n_fga - n_3fga) %>% 
  ungroup() %>% 
  mutate(prop = n_fga/sum(n_fga),
         total = sum(n_fga))
...
shots %>% 
  filter(game_id == "0022400006",
         player1_id == 1628374) %>% 
  group_by(defender_name, is_three) %>%
  summarise(n = n()) %>% 
  ungroup() %>% 
  mutate(prop = n/sum(n),
         total = sum(n))
