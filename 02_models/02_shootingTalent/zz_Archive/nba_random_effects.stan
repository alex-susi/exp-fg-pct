...
functions {
  real partial_sum_lpmf(
    ...
    // random effects (scaled in transformed parameters)
...
data {
  ...
  // standardized log1p(FGA per 36), by shot family
...
parameters {
  ...
  vector[3] mu_volume;  // volume → ability coefficient per shot type
...
transformed parameters {
  ...
  for (t in 1:3) {
    // Volume shift + residual skill (no multiplicative entanglement)
