...
## requests hit the NBA Stats API at once.  4–6 is safe; higher risks
## rate-limiting (HTTP 429) or silent data loss.  Set to 1 to disable.
#N_WORKERS <- 4L
N_WORKERS <- 8L
#N_WORKERS <- 6L

## Per-call delay (seconds) inside each parallel worker.  With N_WORKERS
## workers, the effective request rate is roughly N_WORKERS / response_time.
## The delay primarily staggers the initial burst; 0.3s is sufficient since
## the ~5s API response time dominates steady-state throughput.
CALL_DELAY <- 0.3

## Pause (seconds) between groups in the tracking scrape.  Gives the NBA
## CDN a breather between bursts of activity.  5–10s is enough to reset
## any short-term rate counters without meaningfully affecting wall time.
GROUP_PAUSE <- 5
...
## ════════════════════════════════════════════════════════════════════════════
## .scrape_combos_parallel()  — wave-based parallel dispatch with progress
##
## Splits the combo grid into small chunks (~COMBOS_PER_CHUNK rows each),
## then dispatches in waves of n_workers chunks.  After each wave completes,
## prints a progress bar.  Same total work as a single parLapply, but gives
## live feedback during long scrapes.
##
## Returns bind_rows() of all results (same as the old single-shot approach).
## ════════════════════════════════════════════════════════════════════════════
  ...
  pbp %>%
    ...
    mutate(player1_id      = as.character(player1_id),
           ...
           possession_end = as.integer(fgm == 1 |
                                         ...
                                         is_defrebound == 1 |
                                         period_end == 1|
                                         # jumpball @ period start => possession ends
...
## player's rows come from exactly one game.  This lets us scrape multiple
## dates in a single pass through the combo grid, cutting total API calls
## roughly in half.
##
...
get_game_level_tracking <- function(schedule, season_str, cache_dir = "cache",
  ...
  #    the game_date within each group.
  ...
  for (g_idx in seq_along(date_groups)) {
    ...
    if (use_parallel) {
      ...
      grp_df <- .scrape_combos_parallel(
        combos, N_WORKERS,
        date_mdy    = date_from_mdy,
        season_str  = season_str,
        date_to_mdy = date_to_mdy,
        call_delay  = CALL_DELAY,
        label       = paste0(date_from, " → ", date_to, "  "))
    ...
    if (has_pbp_check) {
      ...
      for (d in grp_dates) {
        ...
        for (att in seq_along(retry_schedule)) {
          ...
          retry_raw <- .scrape_combos_parallel(
            d_combos, rw,
            date_mdy    = d_mdy,
            season_str  = season_str,
            date_to_mdy = d_mdy,
            call_delay  = rd,
            label       = paste0("retry ", att, "  "))
...
build_season_pbp_inChunks <- function(season_year, chunk_size = 5,
  ...
  write_csv(pbp_all %>%
              select(-any_of(c("x", "y", "D", "A", 
                               "T_scaled", "def_dist_scaled"))) %>%
  ...
  write_csv(scaling_params, 
            paste0("scaling_params/scaling_params_", season_tag, ".csv"))
  write_csv(player_map,     
            paste0("player_mapping/player_map_", season_tag, ".csv"))
  write_csv(defteam_map,    
            paste0("team_mapping/defteam_map_", season_tag, ".csv"))
...
## Validate that .build_date_groups produces non-overlapping groups and that
## the number of groups / API calls represents meaningful savings.
if (FALSE) {
  schedule_24 <- get_schedule(2024)
...
## Scrape a small date group with BOTH General Ranges ("" and "Less Than 10 ft"),
## recover game_date via TEAM_ID, de-overlap per date, and verify the results.
if (FALSE) {
  schedule_24 <- get_schedule(2024)
  groups <- .build_date_groups(schedule_24)
